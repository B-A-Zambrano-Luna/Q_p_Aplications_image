...
#from Aux_p_adic_function import  norm_p,FFT_time,G_N_group_aux

def SC_CNN_plot_heat(J, A, B, U, Z, X_0, t, delta_t, Z_k, Name, all_tree=True, size=(8, 8), resolution=300):
    """
    ...
    sc_cnn = SC_CNN()
    A1 = sc_cnn.solution(J, A, B, U, Z, X_0, t, delta_t, Z_k)
    # FFT_A1=FFT_time(A1,G_K)

    # T=list(A1.keys())
    Y = np.array([b for b in Z_k])
    # Y=G_N
    # C_X=[]
    # C_FFT_Y=color_matrix(FFT_A1,G_K)
    ...
    """Show Norm of Fourier transform of output Y(x,t)"""
    ...
    # """ Heat map with sympy of Fourier transform of Y(x,t) """
    # axmatrix = fig_tree.add_axes([0.3, 0.1, 0.6, 0.6])
    # # Plot X(x,y)
    # im = axmatrix.matshow(C_FFT_Y, aspect='auto', origin='lower')
    # axmatrix.set_yticks([])
    # plt.xlabel("time t")
    # plt.title("Fourier transform Y(x,t)")
    # """ Plot colorbar. """
    # axcolor = fig_tree.add_axes([0.95, 0.1, 0.05, 0.6])
    # pylab.colorbar(im, cax=axcolor)
    ...
    # #import os
    # # Name=os.path.basename(__file__)[7:-3]
    # plt.savefig(Name+"_FFT_Y.png", dpi=resolution, bbox_inches='tight')

    """----------------------------------------------"""
    """ Heat maps of A and B"""
    ...
    if type(f) == char_function or type(f) == test_function:
        doc.write("Non-lineality: "+str(f)+"\r\n")
    else:
        doc.write("Non-lineality: "+getsource(f)+"\r\n")
    # Parameters
